...
将判定一个$\text{NP}$-问题的验证器$\mathbb M$在输入$x$和证书$u$上的计算$\mathbb M(x,u)$用布尔公式$\psi_{\mathbb M,x}(u,y,z)$表示。归约是对数空间的，因而是多项式空间的。

其中公式$\psi_{\mathbb{M}, x}(u, y, z)$是一个巨大的逻辑判断，它的变量被分为三组，分别代表计算过程的不同要素：

| 变量组 | 含义          | 作用                                               |
| ------ | ------------- | -------------------------------------------------- |
| $u$    | 证书变量      | 代表证书$u$的每一个比特（这是我们猜想的部分）。    |
| $y$    | 时间/位置变量 | 代表计算过程中的时间步$t$和磁带位置$i$。           |
| $z$    | 状态/内容变量 | 代表在时间$t$、位置$i$处，图灵机的状态和磁带内容。 |
...
设$\text{Ctf}(x)$为满足$\mathbb M(x,u)=1$的证书集合，$\text{Tas}(\psi_{\mathbb M,x}(u,y,z))$为满足$\psi_{\mathbb M,x}(u,y,z)=1$的证书集合，即使$\psi_{\mathbb M,x}(u,y,z)=1$为真的真值指派。根据$\mathbb M(x,u)$计算的唯一性，归约$\psi_{\mathbb M,\_}$是从$\text{Ctf}(x)$到$\text{Tas}(_{\mathbb M,x}(u,y,z))$的双射，其逆函数也是多项式时间可计算的。称满足这些性质的多项式时间归约为**莱文归约**。

搜索问题之间的归约必须是莱文归约。

我的理解是：$\bold{NP}$问题的莱文归约就是把它归约为**搜索验证**，即若第一位的真值$0$不符合条件，则第一位赋为$1$，然后枚举第二位的真值...
...
一个稠密的语言不可能与一个稀疏的语言多项式同构，不然设$L$为稠密，$L'$为稀疏，双射卡普归约函数$r:L\to L'$，则$r(L^{\le n})\subseteq(L')^{\le p(n)}$，前者是指数增长的，后者是多项式增长的。
