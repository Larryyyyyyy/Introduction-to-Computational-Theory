...
第一个蕴含是因为$\bold P$可视为解决$\bold{NP}$问题的图灵机只使用一个迁移函数的特殊情况。

第二个蕴含是因为解决$\bold{EXP}$问题包含不能在多项式时间验证的问题，非确定图灵机对于它们，怎么猜都需要指数时间。
...
第二个我们有$A\in\bold P^B$，$B\in\bold P^C$，因为$B$在多项式图灵机$\mathbb M$调用神谕$C$可以判定的问题里，后者自然可以拿来判定$A$。
...
显然$\mathbb{D}$定义的语言$L \in \bold{SPACE}(g(n))$。
...
设输入图有$n$个结点。从$s$出发，从当前结点$v$猜测一个邻居然后释放$v$所占用的空间，重复$n-1$次，看是否到达$t$。结点的长度是对数的，控制猜测次数（路径长度）的计数器也是对数的，所以Reachability在$\bold{NL}$中。
...
显然$\bold{PSPACE}\subseteq\bold{NPSPACE}$。
...
这是因为所有在$\bold{coNSPACE}(S(n))$的问题都可以用形式一致的$\bold{NSPACE}(S(n))$问题排除所有不符的结果来得到剩余相符的结果。
